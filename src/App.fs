    ...
    label [ ClassName "checkbox-img-label" ]
        [ input
          ...
          img [ ClassName "checkbox-img"
                Src (if isChecked then "img/CheckboxGreen.png" else "img/Checkbox.png") ]
          str text ]

let checkboxImg isChecked status =
    if isChecked then
        match status with
        | Valid -> "img/CheckboxGreen.png"
        | Warning -> "img/CheckboxYellow.png"
        | Invalid -> "img/CheckboxRed.png"
    else
        "img/Checkbox.png"
...
let lazySidebarContent =
    ...
    let sidebarContent model dispatch =
        div [ classModifier "sidebar-content" "open" model.SidebarOpen ]
            ...
            | Fertilizers -> [ span [] [] ]
            | Buy ->
              [ viewSources ToggleBuySource Source.Selected model.BuySourceList model.BuySources dispatch ]
            | Sell ->
               [ viewSources ToggleSellSource Processor.Selected model.SellSourceList model.SellSources dispatch ]
            | Replant
            | Settings -> [ span [] [] ] )
